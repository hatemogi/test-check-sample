...
이 [프로젝트][데모 프로젝트]는, 클로저를 기본으로 속성 기반 테스팅을 소개하고 간단한 데모를 위한 목적으로 만들었습니다. 제가 공부하려는 목적으로 정리했으나, 다른 프로그래밍 언어에서도 유용하게 쓸 수 있는 테스팅 방법이므로, 클로저 개발자가 아니더라도 한 번 읽어봐 주시고, 도움이 된다면 좋겠습니다.
...
앗, 그런데 뭔가가 이상하지요? 데모 프로젝트에서 직접 `lein test`를 해보셨으면 이미 발견하셨을 텐데요, 이 테스트는 실패합니다. 결과 리포트를 보면 이렇게 나옵니다.

```clojure
{:result false, :seed 1466831156583, :failing-size 0, :num-tests 1, :fail [[1]], :shrunk {:total-nodes-visited 1, :depth 0, :result false, :smallest [[0]]}, :test-var "정렬결과-테스트"}
```

처음 보기에 복잡해보입니다만, 요약하면 `:smallest`에 있는 [0]으로 실험해보니 실패했다는 이야기입니다. 이 경우 sort 함수의 문제가 아니라, 테스트를 잘못 작성한 거죠. 흔히 있는 일이죠? 정렬 결과의 앞부분이 **작은지 확인(<)**할 게 아니라, **작거나 같을지(<=)**를 확인해야 합니다. (`:smallest`에 대해서는 잠시 후에 더 자세히 적을게요.)

테스트를 수정하고 다시 돌려보면,

```clojure
{:result true, :num-tests 100, :seed 1466831496398, :test-var "정렬결과-테스트"}
```

이렇게, 정상 통과하는 모습을 볼 수 있습니다. 참고로 `:seed`에는 난수 생성에 사용할 시드값을 보여서, 똑같은 테스트 데이터를 만들어서 다시 돌려볼 수 있도록 도와줍니다.

어떤가요? 괜찮아 보이시나요?

[연습문제: 빈 벡터도 테스트하려면 어떻게 하면 될까요?]
...
이 테스트는, 정렬 함수를 한번 적용하나, 두 번 적용하나 결과는 그대로여야 한다는 속성을 검사합니다.
...
이 함수는 xs 벡터와 ys 벡터를 합친 다음 역순으로 하는 것과, ys 벡터를 역순으로 한 것과 xs 벡터를 역순으로 하고 그 다음으로 합쳐도 두 결과는 같다는 속성을 나타냅니다.
...
```clojure
{:result false, :seed 1466832280883, :failing-size 45, :num-tests 46, :fail [[-15 12 24 25 -16 -12 -45 44 39 -11 24 -22 42 -32 -41 -33 42 16 -27 -5 -6 -21 39 8 -45 -3 38 -19 -19 10 1 6 -23 -21 -35 13 -9 16 -11 -19 -36]], :shrunk {:total-nodes-visited 71, :depth 40, :result false, :smallest [[42]]}, :test-var "실패범위-추려내기"}
```

테스트 리포트를 보시면, `:result false`로 실패했고, `num-test 46`번 실행하다가 실패가 발견 됐습니다. 실패한 테스트 데이타는 `[-15 12 ... -36]`였습니다. 이 데이타로 테스트했을 때 왜 실패했는지를 판단하기 편리하게끔, `test.check`가 입력 데이터의 최소한의 데이터셋을 대신 자동으로 추적해 줍니다. 최소한의 데이터로 줄여 보니 `[42]`일 때 실패했다는 친절한 설명을 해줍니다. 오우~!

그러면, 이 작은 데이타를 기준으로 "왜 실패했는지"를 찾아보고 해결하면 됩니다.

100번 실행하다가 발견됐지만, 임의의 정수들을 담은 경우이므로 문제없이 통과되어 발견되지 않을 수도 있습니다. 100대신 더 큰 수를 넣어서 여러번 테스트하면 매번 확인할 수도 있겠습니다.
